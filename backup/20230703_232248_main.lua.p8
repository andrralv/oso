...
function track_frame()
 ...
 if pl.x > 64 and pl.x< 65 then
  game.shooting_area=true
  spawn_enemies()
 end
...
function track_areas()
 if game.frames.x==4 and game.frames.y==2 then
  game.shooting_area=true
 end 
end
...
function spawn_enemies()
 if game.shooting_area==true then
	 for i=1,3 do
	  _shooter = shooters[i]
	  bullet = make_actor(129,_shooter.x,_shooter.y+1)
	  bullet.dy=0.2
	  bullet.friction=0.05
	 end
	 return nil
	end
...
function track_bullets()
 ...
 _y = game.frames.y * 16
 for a=1,#actor do
  if actor[a].name=="bullet" and a.y>_y then
   del(actor,actor[a])
  end
 end
end
