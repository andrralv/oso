...
function track_frame()
 ...
 --cochinada
 if pl.x>79 and pl.x<80 then
  shooters[3].shot=false
 end
end
...
function track_areas()
 if game.frames.x==5 and game.frames.y==2 then
  spawn_dynamics()
 elseif game.frames.x==6 and game.frames.y==2 then
  spawn_dynamics()
 else
  spawn_dynamics(true)
 end
...
function collide_event(a1,a2)
 ...
 if a1.role=="protag" and a2.name=="ball" then
  game.state="lost"
		game.message="estas muerto"
 end
...
function spawn_enemies()
 ...
 malfin.special_dialog_2={"..ya ahora si", "mae es que estaba jugando", "smash con mi hermanillo", "y se puso a fumar espiral", "se encamoto y saco un chopo", "y me asuste", "en el agua deje un flotador", "talvez en la isla", "alguien pueda ayudar", "..si", "malparido ese", "...", "mae", "jueputa susto ah?", "...", "phew!"}
	...
	shooters[3] = {}
	shooters[3] = make_actor(238,88,24)
	shooters[3].name="shooter"
	shooters[3].bigsprite=true
	shooters[3].can_move=false
	shooters[3].shot=false
end
...
function spawn_dynamics(_reset)
 ...
 debug.active=true
 debug.message=tostr(game.shooting_area) ..game.frames.x ..game.frames.y
	for i=1,3 do
	 _shooter = shooters[i]
...
function track_bullets()
 debug.message=actor[0].name
 if game.frames.y > 2 then
